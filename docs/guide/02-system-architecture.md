...
3. **The frozen aspect schema.** The 9 physical / 11 app aspect names are fixed from Stage B onward. If they ever changed after real extraction started, every downstream artifact (labels, features, trained models, category profiles) would silently misalign. This is why `the pipeline spec` explicitly forbids changing it after extraction begins.
